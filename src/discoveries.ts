...
export type Discovery = {
  art?: ArtSettings; name: string; world: Snapshot; pattern: Pattern; seed: string; palette: PaletteName;
...
export function validateDiscovery(value: unknown): asserts value is Discovery {
  ...
  if (d.art && (!['molten','mist','satin','pixels'].includes(d.art.surface) || !finite(d.art.softness,0,100) || !finite(d.art.relief,0,100) || !finite(d.art.afterglow,0,100) || !finite(d.art.zoom,1,4) || !['none','left','right','center'].includes(d.art.quiet) || !['landscape','square','portrait'].includes(d.art.format))) throw new Error('Invalid art settings');
  if(d.art?.patch)validatePatch(d.art.patch);
  if(d.art?.synthTime!==undefined&&!finite(d.art.synthTime,0,1e10))throw new Error('Invalid synth time');
